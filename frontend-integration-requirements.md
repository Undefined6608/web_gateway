...
- 普通展示查询不得调用账号管理接口，也不得出现密码字段；只有用户主动点击“查看账号密码”并通过管理员二次认证后，才可短暂展示该系统启用账号的明文密码。

查看账号密码交互：

- 点击按钮后打开二次认证弹窗，要求输入本程序的管理员用户名和密码。
- 不要求用户提前登录，也不签发或保存 JWT。
- 验证成功后显示当前系统所有启用账号的角色、账号和密码。
- 密码默认遮罩，可提供单次显示和复制按钮。
...
接口只返回已启用账号。响应设置为不可缓存，前端仍必须在弹窗关闭、系统切换或 60 秒超时后主动清除响应对象。该接口不会返回 JWT，也不会让用户进入管理页面。
