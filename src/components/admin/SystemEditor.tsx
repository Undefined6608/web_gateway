...
type SystemFormValue = Pick<GatewaySystem, 'name' | 'description' | 'owner_id' | 'lifecycle_status'>
...
export function SystemEditor({ open, system, people, onClose, onSaved }: { open: boolean; system: GatewaySystem | null; people: Person[]; onClose: () => void; onSaved: () => void }) {
  ...
  useEffect(() => { if (open) form.setFieldsValue(system ? { ...system } : { lifecycle_status: 'normal', owner_id: null }) }, [open, system, form])
  const submit = async () => {
