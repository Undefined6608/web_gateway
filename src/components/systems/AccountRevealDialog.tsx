...
export function AccountRevealDialog({ systemId, systemName, endpoints }: { systemId: number; systemName: string; endpoints: Endpoint[] }) {
  ...
  const copyCredentials = async (account: RevealedSystemAccount) => {
    const endpoint = endpoints.find(item => item.id === account.endpoint_id)
    const credentials = [`地址：${endpoint ? `${endpoint.endpoint_type === 'online' ? '线上' : '测试'} · ${endpoint.url}` : '未知地址'}`, `角色：${account.role_name}`, `账号：${account.account_name}`, `密码：${account.password}`, account.remark ? `备注：${account.remark}` : ''].filter(Boolean).join('\n')
  ...
  return <>
    ...
    <Modal className="account-reveal-modal" open={open} title={<span className="reveal-modal-title"><SafetyCertificateOutlined />{systemName}账号认证</span>} onCancel={close} footer={null} width={680} destroyOnHidden maskClosable={!loading} keyboard={!loading}>
      {!authenticated ? <div className="reveal-auth-step">
        ...
        {accounts.length ? <div className="revealed-account-list">{accounts.map(account => { const endpoint = endpoints.find(item => item.id === account.endpoint_id); return <div className="revealed-account" key={account.id}>
          <div className="revealed-account-meta"><div className="revealed-account-context"><Tag>{endpoint?.endpoint_type === 'test' ? '测试' : '线上'}</Tag><span>{account.role_name}</span></div><strong>{account.account_name}</strong>{account.remark && <small>{account.remark}</small>}</div>
        ...
        </div>})}</div> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="该系统暂无启用账号" />}
