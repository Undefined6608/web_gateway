...
import { ExclamationCircleFilled, GlobalOutlined, LoadingOutlined, ReloadOutlined } from '@ant-design/icons'
...
export function EndpointRow({ endpoint, onCheck }: { endpoint: Endpoint; onCheck: (id: number) => Promise<void> }) {
  ...
  return <div className="endpoint-row">
    <div className="endpoint-main">
      ...
      <div className="endpoint-url"><a href={endpoint.url} target="_blank" rel="noopener noreferrer"><span className="endpoint-link-text">{endpoint.url}</span><GlobalOutlined /></a><span className="endpoint-check-time">{endpoint.last_checked_at ? `检测于 ${dayjs(endpoint.last_checked_at).format('MM-DD HH:mm:ss')}` : '尚未检测'}</span></div>
    </div>
    <div className="endpoint-health"><Tag icon={meta.icon} color={meta.color}>{meta.label}</Tag><span className="metric">{endpoint.last_http_status ?? '--'} HTTP</span><span className="metric">{endpoint.last_response_time_ms != null ? `${endpoint.last_response_time_ms} ms` : '-- ms'}</span></div>
