...
export function SystemCard({ system, index, onCheck }: { system: GatewaySystem; index: number; onCheck: (systemId: number, endpointId: number) => Promise<void> }) {
  ...
  return <article className={`system-card ${system.lifecycle_status === 'closed' ? 'is-closed' : ''} ${system.colorful_border ? 'has-colorful-border' : ''}`} style={{ '--delay': `${Math.min(index * 55, 330)}ms` } as React.CSSProperties}>
    <div className="system-card-head">
      ...
      <div className="system-card-actions"><div className="system-tags"><Tag className="system-sort-tag">排序值 {system.sort_order}</Tag><Tag className={`system-status system-status-${system.lifecycle_status}`}>{statusMeta[system.lifecycle_status].label}</Tag></div></div>
    </div>
